...
本项目是一个本地优先的多角色群聊讨论网站（暂时支持Windows/MAC一键启动脚本）。你可以像搭建一个研究讨论群一样，为每个角色配置身份、目标、行为准则、发言风格和模型接入方式，让他们围绕同一议题进行短句、连续、对立但有依据的讨论或者多本地Agent协作。
适合以下场景：
...
- 角色可以选择性回复，或者指定其他角色的发言进行反驳
- 优化发言逻辑，多角色群聊表现的会更像现实群聊
